    ...
    for i, v in pairs(game:GetService("Workspace").Enemies:GetChildren()) do
        if v.Name == "Cake Prince" then
            if game:GetService("Workspace").Enemies:FindFirstChild("Ring") or game:GetService("Workspace").Enemies:FindFirstChild("Fist") then
                if (v.HumanoidRootPart.CFrame.Position - plr.Character.HumanoidRootPart.CFrame.Position).Magnitude <= 400 then
                    return true
                end
            end
        end
...
function EWeapon(WP)
    ...
    WP = GetWeapon(WeaponChoose)
    local nigger = plr.Backpack:FindFirstChild(WP)
    if nigger then
        plr.Character.Humanoid:EquipTool(nigger)
    end
end
